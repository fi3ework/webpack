...
register(
	...
	"webpack/lib/library/modern-module/ExternalModule",
	"ModuleExternalInitFragment",
	{
		serialize(obj, { write }) {
			write(obj._request);
			...
			case "module-import": {
				...
				if (connections) {
					...
					for (const connection of connections) {
						const module = connection.originModule;
					...
					for (const module of modules) {
						const importRawDependencies = module.dependencies.filter(
							dep => dep instanceof ImportDependency
						);

						for (const dep of importRawDependencies) {
							if (this.userRequest === dep.request) {
	...
	codeGeneration({
		...
		switch (externalType) {
			...
			default: {
				...
				return {
					sources: new Map([["javascript", null]]),
					runtimeRequirements: EMPTY_RUNTIME_REQUIREMENTS,
					data
				};
			}
