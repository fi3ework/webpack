 ...
 * @property {SyncBailHook<[object, Record<string, string>]>} exportsDefinitions
 */
...
class ConcatenatedModule extends Module {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				exportsDefinitions: new SyncBailHook(["object", "module"])
			};
	...
	codeGeneration({
		...
		if (exportsMap.size > 0) {
			...
			for (const [key, value] of exportsMap) {
				...
				definitionsInHook[key] = value(requestShortener);
			}
			...
			if (!shouldSkipRenderDefinitions) {
				...
				this.buildMeta.exportsFinalName = definitionsInHook;
			}
		}
