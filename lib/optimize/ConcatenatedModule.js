 ...
 * @property {SyncWaterfallHook<[object, ConcatenatedModule]>} exportDefinitions
 */
...
class ConcatenatedModule extends Module {
	...
	static getCompilationHooks(compilation) {
		...
		if (hooks === undefined) {
			hooks = {
				exportDefinitions: new SyncWaterfallHook(["object", "module"])
			};
	...
	codeGeneration({
		...
		if (exportsMap.size > 0) {
			const { exportDefinitions } = ConcatenatedModule.getCompilationHooks(
				this.compilation
			);
			...
			for (const [key, value] of exportsMap) {
				...
				definitionsForHook[key] = value(requestShortener);
			}
			result.add(`\n// EXPORTS\n`);
			result.add(
			...
			exportDefinitions.call(result, definitionsForHook);
		}
