			...
			switch (info.type) {
				case "concatenated": {
					...
					if (
						...
						info.namespaceExportSymbol.startsWith(
							ConcatenationScope.NAMESPACE_OBJECT_EXPORT
						) &&
						info.namespaceExportSymbol.length >
							ConcatenationScope.NAMESPACE_OBJECT_EXPORT.length
					) {
						const name = info.namespaceExportSymbol.slice(
							ConcatenationScope.NAMESPACE_OBJECT_EXPORT.length
						);

						allUsedNames.add(name);
						info.internalNames.set(info.namespaceExportSymbol, name);
						topLevelDeclarations.add(name);
					}
