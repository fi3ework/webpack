					...
					for (const variable of info.moduleScope.variables) {
						...
						if (allUsedNames.has(name) || usedNames.has(name)) {
							...
							for (const identifier of allIdentifiers) {
								...
								if (extraInfo.startsWith("external ")) {
									source.replace(0, source.source().length, "");
								} else {
...
${defineGetters}`
	...
	findNewName(oldName, usedNamed1, usedNamed2, extraInfo) {
		if (extraInfo.startsWith("external ")) {
			const moduleName = extraInfo.slice("external ".length).slice(1, -1);
			return `__WEBPACK_EXTERNAL_MODULE_${moduleName}__`;
		}
