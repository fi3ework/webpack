...
ImportDependency.Template = class ImportDependencyTemplate extends (
	...
	apply(
		...
		const isActualModule = !!dep?.assertions?.actual;
		...
		const hasModuleId = !!moduleGraph.getModule(dep)?.id;
		let moduleId = "";
		if (hasModuleId) {
			// @ts-ignore
			moduleId = moduleGraph.getModule(dep)?.id;
		} else {
			moduleId = dep.request;
		}
		...
		const requestFromMockContent = `new Promise((async (resolve, reject) => {
			const mockFn = __webpack_require__.get_mock("${moduleId}");
			if (mockFn) {
				const mockRes = await mockFn();
				// write to __webpack_cache_module
		...
		source.replace(dep.range[0], dep.range[1] - 1, requestFromMockContent);
	}
};
