		...
		if (inlinedModules) {
			...
			for (const m of inlinedModules) {
				...
				if (renderedModule) {
					...
					if (iife && m.buildMeta.exportsFinalName) {
						startupSource.add(`// ESM COMPAT FLAG\n`);
						startupSource.add(
							runtimeTemplate.defineEsModuleFlagStatement({
								exportsArgument: m.exportsArgument,
						...
						// if iife is not undefined, the modules will be wrapped in an IIFE
						// keep the original exports emit
						if (buildMeta.exportsFinalName) {
							buildMeta.exportsFinalName = null;
						}
					}
